...
import { SuperAdminDashboardAPI } from "../Constant/Api/Api";
...
const SuperAdminDashboard = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
                const r = await ApiHit(SuperAdminDashboardAPI, "GET");
    ...
    const kpis = [
        { label: 'Total Employees', value: s.totalEmployees ?? 0, sub: `${s.activeUsers ?? 0} active users`, icon: <Users size={22} />, color: 'from-blue-500 to-indigo-600', path: '/employees' },
        ...
        { label: 'Monthly Payroll', value: fmt(s.monthlyPayroll), sub: 'active employees', icon: <Wallet size={22} />, color: 'from-purple-500 to-fuchsia-600', path: '/payroll' }
    ];
    ...
    return (
        <main className="p-3 sm:p-4 md:p-6 bg-gray-50 dark:bg-slate-900/40 min-h-screen">
            ...
            {/* Row 2: Growth + Dept Pie + Anniversaries */}
            ...
            <TeamStatusToday
                ...
                subtitle={`Live status of all ${teamStatusToday.length} active user${teamStatusToday.length === 1 ? '' : 's'}.`}
            ...
            {/* Row 3: Recent Leaves + Attendance Donut + Recent Activities */}
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 mb-4 sm:mb-6">
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl p-3 sm:p-4 md:p-5 shadow border border-gray-100 dark:border-slate-800">
                    <div className="flex items-center justify-between mb-4">
